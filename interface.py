    ...
    def process_file(self, file_path):  # Метод для обработки файла
        ...
        if ai_choice == 'да':  # Если пользователь выбрал обработку через ИИ
            ...
            if reference_terms:
                ...
                print(f"{Colors.SUCCESS}Precision: {precision:.2f}{Colors.RESET}")
                print(f"{Colors.SUCCESS}Recall:    {recall:.2f}{Colors.RESET}")
                print(f"{Colors.SUCCESS}F1-score:  {f1:.2f}{Colors.RESET}")
                print("="*80)
            else:
                print(f"{Colors.WARNING}Эталонный файл терминов для этого документа не найден. Метрики не рассчитаны.{Colors.RESET}")
